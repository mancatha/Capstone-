...
# Import the model
model= XGBClassifier()
model=joblib.load(r"model/xgb.joblib")
model_1 = DecisionTreeClassifier()
model_1 = joblib.loaf(r"model/dt.joblib")
...
def predict_customer_attrition(*args, pipeline=decision_tree_pipeline):
...
def predict(text):
    ...
    return "Prediction: " + text
      ...
      # Set up interface
    # Inputs
TENURE= gr.Dropdown(label = "What is the duration of your network?", choices = ['I 18-21 month' 'K > 24 month' 'G 12-15 month' 'J 21-24 month'
 'H 15-18 month' 'F 9-12 month' 'E 6-9 month' 'D 3-6 month'], value= 'I 18-21')
...
REVENUE = gr.Slider(label="What is your monthly income", mininum = 100,maximum=10000, value=200,interactive=True)
ARPU_SEGMENT = gr.Slider(label ="What is your income over 90 days / 3",mininum = 1000,maximum=500000,value=1000,interactive=True)
FREQUENCE = gr.Slider(label= "How do you how often do you use the service ",minimum = 10,maximum=200,value= 5,interactive=True)
DATA_VOLUME = gr.Slider(label= "How many times do you have connections", mininmum =20,maximum = 1000,value= 2,interactive=True)
...
FREQ_TOP_PACK= gr.Slider(label="How many number of times does you been activated to the top pack packages",minimum=10,maximum=1000,value=5,intercative=True)
